...
Task0:
	call	PSwitch		; 启动分页
	jmp	SelectorFlatC:ProcPagingDemo
	mov 	edi, 0
	jmp 	EOI
Task1:
...
OffsetFoo	equ	foo - $$
	...
	mov	al, 'H'
	mov	[gs:((80 * 14 + 0) * 2)], ax	; 屏幕第 17 行, 第 0 列。
	mov	al, 'U'
	mov	[gs:((80 * 14 + 1) * 2)], ax	; 屏幕第 17 行, 第 1 列。
	mov	al, 'S'
	mov	[gs:((80 * 14 + 2) * 2)], ax	; 屏幕第 17 行, 第 2 列。
	mov	al, 'T'
	mov	[gs:((80 * 14 + 3) * 2)], ax	; 屏幕第 17 行, 第 3 列。
	jmp $
LenFoo	equ	$ - foo
; ---------------------------------------------------------------------------
