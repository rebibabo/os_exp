...
_dwCurrentTask		dd	0
_dwCurrentIdx		dd 	0
_priority			dd  4, 6, 2, 5			; 任务优先级
_time 				dd  4, 6, 2, 5			; 任务还能执行的时间片
_idx 				dd  0, 1, 2, 3
...
dwCurrentIdx	equ _dwCurrentIdx - $$
...
.exit:      
	mov		ah, 05h							; 0000: 黑底    1111: 白字
	mov 	al, [dwCurrentTask]
    add 	al, '0'
	mov		[gs:((80 * 15 + 0) * 2)], ax	; 屏幕第 20 行, 第 0 列。
	mov 	al, [time]
    add 	al, '0'
	mov		[gs:((80 * 15 + 2) * 2)], ax	; 屏幕第 20 行, 第 0 列。
	mov 	al, [time + 4]
    add 	al, '0'
	mov		[gs:((80 * 15 + 4) * 2)], ax	; 屏幕第 20 行, 第 0 列。
	mov 	al, [time + 8]
    add 	al, '0'
	mov		[gs:((80 * 15 + 6) * 2)], ax	; 屏幕第 20 行, 第 0 列。
	mov 	al, [time + 12]
    add 	al, '0'
	mov		[gs:((80 * 15 + 8) * 2)], ax	; 屏幕第 20 行, 第 0 列。
...
showInfo:
	...
	mov		al, byte [time]
	add 	al, '0'
	mov		[gs:((80 * 17 + 0) * 2)], ax	; 屏幕第 20 行, 第 0 列。
	mov		al, byte [time + 4]
	add 	al, '0'
	mov		[gs:((80 * 17 + 2) * 2)], ax	; 屏幕第 20 行, 第 1 列。
	mov		al, byte [time + 8]
	add 	al, '0'
	mov		[gs:((80 * 17 + 4) * 2)], ax	; 屏幕第 20 行, 第 2 列。
	mov		al, byte [time + 12]
	add 	al, '0'
	mov		[gs:((80 * 17 + 6) * 2)], ax	; 屏幕第 20 行, 第 3 列。

	mov		al, byte [idx]
	add 	al, '0'
	mov		[gs:((80 * 18 + 0) * 2)], ax	; 屏幕第 20 行, 第 0 列。
	mov		al, byte [idx + 4]
	add 	al, '0'
	mov		[gs:((80 * 18 + 2) * 2)], ax	; 屏幕第 20 行, 第 1 列。
	mov		al, byte [idx + 8]
	add 	al, '0'
	mov		[gs:((80 * 18 + 4) * 2)], ax	; 屏幕第 20 行, 第 2 列。
	mov		al, byte [idx + 12]
	add 	al, '0'
	mov		[gs:((80 * 18 + 6) * 2)], ax	; 屏幕第 20 行, 第 3 列。
	pop 	eax
; End of showInfo -----------------------------------------------------------
...
.init:
	...
	xor 	edx, edx
.start:
	mov 	ecx, dword [time + eax];
	mov 	ebx, dword [time + eax + 4];
	add 	edx, 1
	cmp 	ecx, ebx
	jge 	.end
	mov 	dword [time + eax], ebx
	mov 	dword [time + eax + 4], ecx
	mov 	ebx, dword [idx + eax + 4]
	mov 	ecx, dword [idx + eax]
	mov 	dword [idx + eax], ebx
	mov 	dword [idx + eax + 4], ecx
.end:
	add 	eax, 4
	cmp 	edx, esi
	jl 		.start
